...
func (c *Metadata) Update(p *ConnectionPoolWrapper) error {
	db := p.GetConnection()
...
func (c *Metadata) Add(p *ConnectionPoolWrapper) error {
	db := p.GetConnection()
	defer p.ReleaseConnection(db)
	_, err := db.Exec("INSERT INTO metadata(object_type, object_id, data_key, data_value) VALUES($1, $2, $3, $4);",
		c.Object_type, c.Object_id, c.Data_key, c.Data_value)
	return err
}

func (c *Metadata) Del(p *ConnectionPoolWrapper) error {
	db := p.GetConnection()
	defer p.ReleaseConnection(db)
...
func (c *Metadata) Get(p *ConnectionPoolWrapper) error {
	db := p.GetConnection()
	defer p.ReleaseConnection(db)
